import{Bn as hl,St as Rh,f as D,n as $i,rr as li,tn as _D,y as ET}from"./chunk-XMMDyd3T.js";import"./chunk-DjQzzoZv.js";import{n as Ce}from"./chunk-CvuArTVg.js";import{Lt as n4}from"./main-ADO3GQQL.js";import{g as kr}from"./chunk-BUz2QEmb.js";var u={name:`si6_accounting_data`,script_type:`API`,api_method:`si6_accounting_data`,script:`
...
for group_name in ["Products", "Services"]:
    item_group_name = frappe.db.get_value(
        "Item Group", {"item_group_name": group_name}, "name"
    )
    if not item_group_name:
        skipped.append(f"Item Group: {group_name} (not found for update)")
        continue

    current_is_group = frappe.db.get_value("Item Group", item_group_name, "is_group")
    if frappe.utils.cint(current_is_group) != 1:
        frappe.db.set_value("Item Group", item_group_name, "is_group", 1)
        created.append(f"Item Group: {group_name} (is_group set to 1)")
    else:
        skipped.append(f"Item Group: {group_name} (already is_group=1)")
...
frappe.response["message"] = {
    ...
    `};var w={name:`si6_stock`,script_type:`API`,api_method:`si6_stock`,script:`
# Frappe Server Script \u2014 API Type
# Method Name: si6_stock
# Endpoint: GET /api/method/si6_stock
#
# Returns projected stock rows without using query reports.
# Mirrors ERPNext stock_projected_qty core logic at API level.

def has_bin_read_access():
	# Some Server Script sandboxes do not expose frappe.has_permission.
	# Use a safe probe through get_list (which enforces permissions).
	try:
		frappe.get_list("Bin", fields=["name"], limit=1)
		return True
	except:
		return False


if not has_bin_read_access():
	frappe.throw("Insufficient permission to read stock bins")


def make_in_clause(values_list, key_prefix, sql_values):
	placeholders = []
	for idx, value in enumerate(values_list):
		key = key_prefix + frappe.utils.cstr(idx)
		sql_values[key] = value
		placeholders.append("%(" + key + ")s")
	return ", ".join(placeholders)


def build_item_map(item_codes, item_code_filter, brand_filter):
	if not item_codes:
		return {}
	...
	where_clauses = [
		"i.is_stock_item = 1",
		"i.disabled = 0",
		"(i.end_of_life IS NULL OR i.end_of_life = '' OR i.end_of_life = '0000-00-00' OR i.end_of_life >= %(today)s)",
	]

	if item_code_filter:
		where_clauses.append("i.item_code = %(item_code_filter)s")
		values["item_code_filter"] = item_code_filter

	if brand_filter:
		where_clauses.append("i.brand = %(brand_filter)s")
		values["brand_filter"] = brand_filter

	in_clause = make_in_clause(item_codes, "item_code_", values)
	where_clauses.append("i.name IN (" + in_clause + ")")
...
def build_reorder_map(item_codes):
	if not item_codes:
		return {}

	values = {}
	in_clause = make_in_clause(item_codes, "reorder_item_", values)
	reorder_sql = (
		"""
		SELECT
			ir.parent,
	...
	for row in reorder_rows:
		key = (row.parent or "") + "||" + (row.warehouse or "")
		reorder_map[key] = row
...
def is_truthy(value):
	text = frappe.utils.cstr(value or "").strip().lower()
	return text in ["1", "true", "yes", "y", "on"]


def add_reserved_qty_rows(pos_map, rows):
	for row in rows:
		key = (row.item_code or "") + "||" + (row.warehouse or "")
		pos_map[key] = frappe.utils.flt(pos_map.get(key)) + frappe.utils.flt(row.reserved_qty_for_pos)


def build_pos_reserved_rows(item_codes, company_filter, warehouse_bounds_filter, child_table, qty_field):
	if not item_codes:
		return []

	values = {}
	where_clauses = [
		"COALESCE(pi.consolidated_invoice, '') = ''",
		"COALESCE(pi.is_return, 0) = 0",
		"child.docstatus = 1",
	]

	if company_filter:
		where_clauses.append("pi.company = %(company_filter)s")
		values["company_filter"] = company_filter

	warehouse_expr = "COALESCE(NULLIF(child.warehouse, ''), pi.set_warehouse)"

	if warehouse_bounds_filter:
		where_clauses.append("w.lft >= %(warehouse_lft)s")
		where_clauses.append("w.rgt <= %(warehouse_rgt)s")
		values["warehouse_lft"] = warehouse_bounds_filter.lft
		values["warehouse_rgt"] = warehouse_bounds_filter.rgt

	in_clause = make_in_clause(item_codes, "pos_item_", values)
	where_clauses.append("child.item_code IN (" + in_clause + ")")
	where_sql = " AND ".join(where_clauses)
	pos_sql = (
		"""
		SELECT
		...
		+ """ AS warehouse,
			SUM(COALESCE(child."""
		+ qty_field
		+ """, 0)) AS reserved_qty_for_pos
		FROM \`tab"""
		+ child_table
		+ """\` child
		INNER JOIN \`tabPOS Invoice\` pi ON pi.name = child.parent
		INNER JOIN \`tabWarehouse\` w ON w.name = """
		+ warehouse_expr
		+ """
		WHERE
		"""
...
def build_pos_reserved_map(item_codes, company_filter, warehouse_bounds_filter):
	if not item_codes:
		return {}

	pos_map = {}
	add_reserved_qty_rows(
		pos_map,
		build_pos_reserved_rows(
			item_codes,
			company_filter,
			warehouse_bounds_filter,
			"POS Invoice Item",
			"stock_qty",
		),
	)
...
company = frappe.utils.cstr(frappe.form_dict.get("company") or "").strip()
warehouse = frappe.utils.cstr(frappe.form_dict.get("warehouse") or "").strip()
item_code = frappe.utils.cstr(frappe.form_dict.get("item_code") or "").strip()
brand = frappe.utils.cstr(frappe.form_dict.get("brand") or "").strip()
recalculate_pos_raw = frappe.form_dict.get("recalculate_pos_reservation")
if recalculate_pos_raw is None:
	recalculate_pos_raw = frappe.form_dict.get("recalculate_pos")

recalculate_pos_reservation = True
if recalculate_pos_raw is not None:
	recalculate_pos_reservation = is_truthy(recalculate_pos_raw)
...
warehouse_bounds = frappe.db.get_value(
	"Warehouse",
	warehouse,
	["lft", "rgt", "company"],
	as_dict=True,
...
if not warehouse_bounds:
	frappe.throw("Invalid warehouse")

warehouse_company = frappe.utils.cstr(warehouse_bounds.company or "").strip()
if warehouse_company and warehouse_company != company:
	frappe.throw("warehouse does not belong to the provided company")

bin_where = ["1=1"]
bin_values = {}

if item_code:
	bin_where.append("b.item_code = %(item_code)s")
	bin_values["item_code"] = item_code

bin_where.append("w.company = %(company)s")
bin_values["company"] = company

bin_where.append("w.lft >= %(warehouse_lft)s")
bin_where.append("w.rgt <= %(warehouse_rgt)s")
bin_values["warehouse_lft"] = warehouse_bounds.lft
bin_values["warehouse_rgt"] = warehouse_bounds.rgt

where_sql = " AND ".join(bin_where)
bin_sql = (
	"""
	SELECT
		b.item_code,
		...
		COALESCE(b.reserved_qty_for_production, 0) AS reserved_qty_for_production,
		COALESCE(b.reserved_qty_for_production_plan, 0) AS reserved_qty_for_production_plan,
		COALESCE(b.reserved_qty_for_sub_contract, 0) AS reserved_qty_for_sub_contract,
...
if not bin_rows:
	...
	item_codes = []
	for row in bin_rows:
		if row.item_code and row.item_code not in item_codes:
			item_codes.append(row.item_code)

	item_map = build_item_map(item_codes, item_code, brand)
	reorder_map = build_reorder_map(item_codes)
	pos_reserved_map = {}
	if recalculate_pos_reservation:
		pos_reserved_map = build_pos_reserved_map(item_codes, company, warehouse_bounds)
	...
	for row in bin_rows:
		...
		reorder_key = (row.item_code or "") + "||" + (row.warehouse or "")
		reorder_row = reorder_map.get(reorder_key) or {}

		reserved_qty_for_pos = 0
		projected_qty = frappe.utils.flt(row.projected_qty)
		if recalculate_pos_reservation:
			reserved_qty_for_pos = frappe.utils.flt(pos_reserved_map.get(reorder_key))
			projected_qty = projected_qty - reserved_qty_for_pos
		re_order_level = frappe.utils.flt(reorder_row.get("warehouse_reorder_level"))
		re_order_qty = frappe.utils.flt(reorder_row.get("warehouse_reorder_qty"))

		shortage_qty = 0
		if (re_order_level or re_order_qty) and re_order_level > projected_qty:
			shortage_qty = re_order_level - projected_qty
		...
		data.append(
			{
				...
				"actual_qty": frappe.utils.flt(row.actual_qty),
				"planned_qty": frappe.utils.flt(row.planned_qty),
				"indented_qty": frappe.utils.flt(row.indented_qty),
				"ordered_qty": frappe.utils.flt(row.ordered_qty),
				"reserved_qty": frappe.utils.flt(row.reserved_qty),
				"reserved_qty_for_production": frappe.utils.flt(row.reserved_qty_for_production),
				"reserved_qty_for_production_plan": frappe.utils.flt(row.reserved_qty_for_production_plan),
				"reserved_qty_for_sub_contract": frappe.utils.flt(row.reserved_qty_for_sub_contract),
...
# - transaction_date range filter (from/to ISO8601)
...
def parse_iso8601_date(value):
    raw = frappe.utils.cstr(value or "").strip()
    if not raw:
        return None

    # Accept full ISO8601 (datetime) and plain date formats.
    date_part = raw.split("T")[0]
    try:
        return frappe.utils.getdate(date_part)
    except Exception:
        return None
...
from_date = parse_iso8601_date(
    frappe.form_dict.get("from") or frappe.form_dict.get("from_date")
)
to_date = parse_iso8601_date(
    frappe.form_dict.get("to") or frappe.form_dict.get("to_date")
)
...
if from_date:
    where_clauses.append("so.transaction_date >= %(from_date)s")
    values["from_date"] = from_date

if to_date:
    where_clauses.append("so.transaction_date <= %(to_date)s")
    values["to_date"] = to_date
...
frappe.response["message"] = {
    ...
    `};var Q=(()=>{class t{constructor(){this.crudStore=D(kr),this.router=D(Ce),this.scripts=[u,f,h,v,b,w,g,y,E,S],this.initialization=ET({defaultValue:[],loader:async()=>{let a=[],e=await this.crudStore.list(`Server Script`,a,[],[],100);return e.length&&await Promise.all(e.map(s=>this.crudStore.delete(`Server Script`,s.name))),await Promise.all(this.scripts.map(s=>this.crudStore.create(`Server Script`,s))),await this.crudStore.list(`Server Script`,a,[],[],100)}}),li(()=>{!this.initialization.isLoading()&&!this.initialization.error()&&this.router.navigateByUrl(`/documents-preparation`)})}static{this.ɵfac=function(e){return new(e||t)}}static{this.ɵcmp=_D({type:t,selectors:[[`app-initialization`]],decls:4,vars:0,consts:[[1,`relative`,`h-screen`,`flex`,`items-center`,`justify-center`],[`src`,`assets/imgs/sic.svg`,`alt`,`Strategic Innovation Center`,1,`absolute`,`top-0`,`w-1/4`,`opacity-75`]],template:function(e,s){e&1&&($i(0,`div`,0),Rh(1,`img`,1),$i(2,`div`),Rh(3,`ion-spinner`),hl()())},dependencies:[n4],encapsulation:2})}}return t})();export{Q as InitializationComponent};